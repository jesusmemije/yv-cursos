...
class MainIndexController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (file_exists(storage_path('installed'))) {
            ...
            // event_category_id = 1 ==> Corresponde a transmisiones en vivo
            $data['nextEvent'] = Event::where('event_category_id', 1)
                ->where('start', '>=', Carbon::now())
                ->active()
                ->orderBy('start', 'asc')
                ->first();

            $data['nextCourse'] = Event::where('event_category_id', '!=', 1)
                ->where('start', '>=', Carbon::now())
                ->active()
                ->orderBy('start', 'asc')
                ->first();
            ...
            $existingCount = $images->count();
            $remaining = max(0, 4 - $existingCount);

            // Generar datos ficticios para los registros que faltan
            for ($i = 0; $i < $remaining; $i++) {
                $images->push(new MainImage(['image_url' => "frontend/assets/img/no-image.jpg", 'redirect_url' => "#"]));
            }

            $data['mainImages'] = $images->concat($images);
