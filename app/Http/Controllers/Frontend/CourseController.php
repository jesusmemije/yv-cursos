...
class CourseController extends Controller
{
    ...
    public function allCourses(Request $request)
    {
        ...
        $recordsCourse = Course::withCount('enrollments')
            ->orderBy('enrollments_count', 'desc')
            ->where('private_mode', '!=', 1)
            ->active()
            ->take(3)
            ->get();
        $data['bestsellerCourse'] = $recordsCourse->concat($recordsCourse);
    ...
    public function categoryCourses($slug)
    {
        ...
        $bestsellerCourse = Course::withCount('enrollments')
            ->orderBy('enrollments_count', 'desc')
            ->where('private_mode', '!=', 1)
            ->whereCategoryId($data['category']->id)
            ->active()
            ->take(3)
            ->get();

        // Verificar si hay menos de 6 blogs
        while ($bestsellerCourse->count() < 6) {
            // Añadir una copia de los blogs existentes al final
            $bestsellerCourse = $bestsellerCourse->concat($bestsellerCourse);
        }
        $data['bestsellerCourses'] = $bestsellerCourse->take(6);

        $data['availableCourses'] = Course::where('private_mode', '!=', 1)
            ->whereCategoryId($data['category']->id)
            ->active()
            ->inRandomOrder()
            ->take(3)
    ...
    public function allDiplomas(Request $request)
    {
        $slug = "Diplomados-y-Formación-Avanzada";
        $tagId = Tag::whereSlug("Cursos-Complementarios")->first()->id;
        $data['pageTitle'] = "Courses";
        $data['metaData'] = staticMeta(4);
        $data['categories'] = Category::with(['courses' => function ($query) {
            $query->orderBy('title', 'asc');
        ...
        $data['category'] = Category::whereSlug($slug)->firstOrFail();
        $data['courses'] = Course::where('private_mode', '!=', 1)
            ->whereHas('tags', function ($query) use ($tagId) {
                $query->where('tag_id', $tagId); // Filtrar por el tag_id específico
            })
            ->active()
            ->paginate($this->coursePaginateValue);
        ...
        return view('frontend.course.diploma-list', $data);
    }
