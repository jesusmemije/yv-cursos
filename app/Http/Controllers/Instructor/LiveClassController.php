...
class LiveClassController extends Controller
{
    ...
    public function liveClassIndex(Request $request, $course_uuid)
    {
        ...
        // Obtener la configuración de Zoom del usuario para obtener la zona horaria
        $zoom = ZoomSetting::whereUserId(auth()->id())->first();
        $userTimezone = $zoom ? $zoom->timezone : 'UTC'; // Usar UTC como fallback si no se encuentra la zona horaria

        $nowInUserTimezone = now($userTimezone);

        // Clasificar las clases en vivo como próximas, actuales y pasadas según la zona horaria del usuario
        $data['upcoming_live_classes'] = LiveClass::whereCourseId($data['course']->id)->whereUserId(Auth::user()->id)
            ->where(function($q) use ($nowInUserTimezone) {
                $q->whereDate('date', '>=', $nowInUserTimezone->toDateString())
                ->whereTime('time', '>', $nowInUserTimezone->toTimeString());
            })
        ...
        $data['current_live_classes'] = LiveClass::whereCourseId($data['course']->id)->whereUserId(Auth::user()->id)
            ->where(function($q) use ($nowInUserTimezone) {
                $q->whereDate('date', $nowInUserTimezone->toDateString())
                ->whereTime('time', '<=', $nowInUserTimezone->toTimeString())
                ->whereTime(DB::raw('SEC_TO_TIME((duration * 60) + TIME_TO_SEC(time))'), '>=', $nowInUserTimezone->toTimeString());
            })
            ->latest()->paginate(15, '*', 'current');

        $data['past_live_classes'] = LiveClass::whereCourseId($data['course']->id)->whereUserId(Auth::user()->id)
            ->where(function($q) use ($nowInUserTimezone) {
                $q->whereDate('date', '<', $nowInUserTimezone->toDateString())
                ->orWhere(function($q) use ($nowInUserTimezone) {
                    $q->whereDate('date', $nowInUserTimezone->toDateString())
                        ->whereTime(DB::raw('SEC_TO_TIME((duration * 60) + TIME_TO_SEC(time))'), '<', $nowInUserTimezone->toTimeString());
                });
            })
    ...
    public function getZoomMeetingLink(Request $request)
    {
        ...
        $meeting = \Jubaer\Zoom\Facades\Zoom::createMeeting([
            ...
            "timezone" => @$zoom->timezone ?? 'America/Mexico_City', // set your timezone
