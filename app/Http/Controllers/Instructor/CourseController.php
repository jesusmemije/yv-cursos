...
class CourseController extends Controller
{
    ...
    public function updateCategory(Request $request, $uuid)
    {
        ...
        if ($request->video) {
            Log::info('Video File: '.$request->video->getClientOriginalName());
    ...
    {
        $file = $request->file('csv_file'); // Asumiendo que el archivo CSV se envía desde un formulario

        // Abre el archivo CSV
        if (($handle = fopen($file->getPathname(), 'r')) !== false) {
            // Leer la primera línea del archivo CSV para obtener los encabezados
            $headers = fgetcsv($handle, 1000, ',');

            // Mapear los encabezados a los índices
            $headerIndex = array_flip($headers);

            // Definir las propiedades del modelo Course que queremos importar
            $propertiesToImport = [
                'id',
                'title',
                'subtitle',
                ...
                'image',
                'video',
                // No incluir 'slug' aquí porque lo generaremos más adelante
                'user_id',
                'course_type',
                'instructor_id',
                'category_id',
            ...
            while (($row = fgetcsv($handle, 1000, ',')) !== false) {
                DB::transaction(function () use ($row, $headerIndex, $propertiesToImport) {
                    $course = new Course();

                    foreach ($propertiesToImport as $property) {
                        if (isset($headerIndex[$property]) && isset($row[$headerIndex[$property]])) {
                            $course->$property = $row[$headerIndex[$property]];
                        }
                    }

                    // Generar slug
                    if (Course::where('slug', Str::slug($course->title))->count() > 0) {
                        $course->slug = Str::slug($course->title) . '-' . rand(100000, 999999);
                    } else {
                        $course->slug = Str::slug($course->title);
                    }

                    if (!empty($course->image)) {
                        $course->image = $this->saveImageFromUrl('meta', $course->image, 575, 450, 'fit');
                    }

                    $course->save();

                    // Crear o actualizar la relación CourseInstructor
                    CourseInstructor::updateOrCreate(
                        ['course_id' => $course->id, 'instructor_id' => $course->user_id],
                        ['share' => 100, 'status' => STATUS_ACCEPTED]
                    );
                });
            }
        ...
        return back()->with('success', 'Cursos importados correctamente.');
    }
}
