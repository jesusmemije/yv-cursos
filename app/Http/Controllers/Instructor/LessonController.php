...
//        $lecture->file_size = number_format(File::size($lecture->file_path) / 1048576, 2);
    ...
    public function importLessonsWithLectures(Request $request)
    {
        ...
        if (($handle = fopen($file->getPathname(), 'r')) !== false) {
            ...
            $chunkSize = 100; // Define el tamaño del chunk
            $rows = [];

            // Lee cada línea del archivo CSV después de la cabecera
            while (($row = fgetcsv($handle, 1000, ',')) !== false) {
                $rows[] = array_combine($header, $row);

                if (count($rows) === $chunkSize) {
                    // Despacha un trabajo para procesar este chunk
                    ProcessImportChunk::dispatch($rows);
                    $rows = []; // Restablece el array para el siguiente chunk
                }
            }
        ...
        return back()->with('success', 'La importación de lecciones y conferencias está en proceso.');
    }
    ...
    //             // Buscar o crear la lección basada en el nombre y el id del curso
    ...
    //                 $this->importLectureVideo($lecture, $rowData['video_url']); // Save Video File, Path, Size, Duration
    //             } elseif ($rowData['type'] == 'text') {
    //                 $lecture->text = $rowData['text'];
    //             } elseif ($rowData['type'] == 'pdf') {
    //                 $lecture->file_path = $rowData['file_path']; // asumiendo que esto se manejará adecuadamente en el almacenamiento de archivos
    //             }
    ...
    //     return back()->with('success', 'Lecciones y conferencias importadas correctamente.');
    // }
    ...
    //         $file_details = $this->uploadFileWithDetails('video', new \Illuminate\Http\File($downloadedVideo['tempPath']));
    //         if ($file_details['is_uploaded']) {
    //             $lecture->file_path = $file_details['path'];
    //             $lecture->file_duration_second = (int)$file_duration_second;
    ...
    //         unlink($downloadedVideo['tempPath']);
    //     }
