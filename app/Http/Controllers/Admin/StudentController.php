    ...
    public function importStudents(Request $request)
    {
        ...
        if (($handle = fopen($file->getPathname(), 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ',')) !== false) {
                DB::transaction(function () use ($data, $headerIndex) {
                    $user = new User();

                    // Mapear datos del CSV al modelo User
                    $user->id = $data[$headerIndex['id']];
                    $user->name = $data[$headerIndex['first_name']] . ' ' . $data[$headerIndex['last_name']];
                    $user->email = $data[$headerIndex['email']];
                    $user->password = Hash::make($data[$headerIndex['password']]);
                    $user->mobile_number = !empty($data[$headerIndex['phone_number']]) ? $data[$headerIndex['phone_number']] : null;
                    $user->address = $data[$headerIndex['address']];
                    $user->role = 3; // Asumir rol de estudiante
                    $user->email_verified_at = now();

                    // Guardar el usuario
                    $user->save();

                    // Agregar datos específicos del estudiante
                    $student_data = [
                        'user_id' => $user->id,
                        'first_name' => $data[$headerIndex['first_name']],
                        'last_name' => $data[$headerIndex['last_name']],
                        'address' => $data[$headerIndex['address']],
