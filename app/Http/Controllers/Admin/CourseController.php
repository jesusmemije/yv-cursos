...
class CourseController extends Controller
{
    ...
    public function courseEnrollImport(Request $request)
    {
        $request->validate([
        ...
        if (($handle = fopen($path, 'r')) !== false) {
            // Leer la primera línea para omitir la cabecera
            $headers = fgetcsv($handle, 1000, ',');

            while (($data = fgetcsv($handle, 1000, ',')) !== false) {
                $user_id = $data[0];
                $course_id = $data[1];
                $expired_after_days = $data[2];

                // Iniciar la transacción
                DB::transaction(function () use ($user_id, $course_id, $expired_after_days) {
                    ...
                    $orderItem = new Order_item([
                        ...
                        'user_id' => $user_id,
                        'course_id' => $course_id,
                        'owner_user_id' => $course->user_id ?? null,
                        'unit_price' => 0,
                        'admin_commission' => 0,
                        'owner_balance' => 0,
                        'sell_commission' => 0
                    ...
                    $enrollment = new Enrollment([
                        ...
                        'course_id' => $course_id,
                        'order_id' => $order->id,
                        'status' => 1, // Asumiendo 1 como estado activo
                        'start_date' => now(),
                        'end_date' => ($course->access_period) ? Carbon::now()->addDays($course->access_period) : MAX_EXPIRED_DATE
                    ]);
                    $enrollment->save();
                });
            }
        ...
        return back()->with('success', 'Inscripciones importadas correctamente.');
    }
}
