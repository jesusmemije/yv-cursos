...
class CartManagementController extends Controller
{
    ...
    public function pay(Request $request)
    {
        ...
        if ($request->payment_method == 'openpay') {
            if (empty(env('OPENPAY_MERCHANT_ID')) || empty(env('OPENPAY_PRIVATE_KEY'))) {
                $this->showToastrMessage('error', __('Openpay payment gateway is off!'));
                return redirect()->back();
            }
        }
        ...
        if ($request->payment_method == PAYPAL) {
            ...
            if($responseData['success']){
                ...
                $this->showToastrMessage('error', 'Error: Transacción no exitosa');
                return redirect()->back();
            }
        ...
        } else if ($request->payment_method == OPENPAY)  {
            $total = $order->grand_total * (get_option('openpay_conversion_rate') ? get_option('openpay_conversion_rate') : 0);
            $total = number_format($total, 2,'.','');
            $user = auth()->user();
            $object = [
                'id' => $order->uuid,
                'payment_method' => OPENPAY,
                'currency' => get_option('openpay_currency'),
            ];
            $post_data = [
