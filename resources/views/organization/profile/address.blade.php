...
@section('breadcrumb')
    <div class="page-banner-content text-center">
        <h3 class="page-banner-heading text-black pb-15"> {{ __('Profile') }} </h3>
...
@section('content')
    <div class="instructor-profile-right-part">
        <form method="POST" action="{{ route('organization.address.update', [$organization->uuid]) }}"
            enctype="multipart/form-data">
            @csrf
            <div class="instructor-profile-info-box">
                <div class="instructor-my-courses-title d-flex justify-content-between align-items-center">
                    <h6>{{ __('Address and Location') }}</h6>
                </div>
                <div class="row">
                        ...
                        <select name="country_id" id="country_id" class="form-select">
                            ...
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}"
                                    ...
                                    @else
                                    {{ $organization->country_id == $country->id ? 'selected' : '' }} @endif
                ...
                <div class="row">
                                ...
                                @if ($organization->country)
                                    @foreach ($organization->country->states as $selected_state)
                                        <option value="{{ $selected_state->id }}"
                                            {{ $organization->state_id == $selected_state->id ? 'selected' : '' }}
                                ...
                                @if ($organization->state)
                                    @foreach ($organization->state->cities as $selected_city)
                                        <option value="{{ $selected_city->id }}"
                                            {{ $organization->city_id == $selected_city->id ? 'selected' : '' }}
                    ...
                    <div class="col-md-4 mb-30">
                        ...
                        <input type="text" name="postal_code" value="{{ $organization->postal_code }}"
