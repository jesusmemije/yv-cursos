            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="customers__area bg-style mb-30">
                        ...
                        <form action="{{route('event.update', [$event->uuid])}}" method="post" class="form-horizontal" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')

                            <div class="input__group mb-25">
                                <label>{{__('Title')}} <span class="text-danger">*</span></label>
                                <input type="text" name="title" value="{{$event->title}}" placeholder="{{__('Title')}}" class="form-control slugable"  onkeyup="slugable()">
                                @if ($errors->has('title'))
                            ...
                            <div class="input__group mb-25">
                                ...
                                <input type="text" name="slug" value="{{$event->slug}}" placeholder="{{__('Slug')}}" class="form-control slug" onkeyup="getMyself()">
                                @if ($errors->has('slug'))
                            ...
                            <div class="input__group mb-25">
                                ...
                                <select name="status" id="status">
                                    ...
                                    <option value="1" @if($event->status == 1) selected @endif>{{ __('Published') }}</option>
                                    <option value="0" @if($event->status != 1) selected @endif>{{ __('Unpublished') }}</option>
                                </select>
                            </div>
                            ...
                            <div class="input__group mb-25">
                                ...
                                <select name="event_category_id" class="form-control">
                                    ...
                                    @foreach($eventCategories as $category)
                                        <option value="{{ $category->id }}" @if($category->id == $event->event_category_id) selected @endif>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input__group mb-25">
                                ...
                                <select name="tag_ids[]" class="form-control multiple-basic-single" multiple>
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" {{ in_array($tag->id, $eventTags) ? 'selected' : null }}>{{ $tag->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="input__group mb-25">
                                ...
                                <select name="time_zone" class="form-control select2">
                                    @foreach(getTimeZonesList() as $key => $value)
                                        <option value="{{ $key }}" {{ $key == $event->time_zone ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input__group mb-25">
                                ...
                                <input type="text" name="location" value="{{$event->location}}" placeholder="{{__('Event Location')}}" class="form-control">
                            ...
                            <div class="input__group mb-25">
                                ...
                                <textarea name="details" id="summernote">{{$event->details}}</textarea>
