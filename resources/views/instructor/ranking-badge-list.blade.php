...
@section('breadcrumb')
    <div class="page-banner-content text-center">
        <h3 class="page-banner-heading text-black pb-15"> {{ __('Ranking Badge') }} </h3>
    ...
    <div class="instructor-profile-right-part">
        <div class="instructor-quiz-list-page instructor-ranking-badge-page">
            ...
            <div class="instructor-my-courses-title d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('instructor.dashboard') }}" class="theme-btn theme-button1 default-back-btn default-hover-btn m-0">{{ __('Back') }}</a>
            </div>
                        ...
                        <div class="ranking-item d-flex align-items-center justify-content-between">
                            ...
                            <div class="d-flex align-items-center ranking-item-right">
                                <div class="flex-shrink-0">
                                    ...
                                    <p class="font-15 color-heading text-center">
                                        @if(get_currency_placement() == 'after')
                                            {{ $level->from }} {{ get_currency_symbol() }}
                                        @else
                                            {{ get_currency_symbol() }} {{ $level->from }}
                                        @endif
                                    </p>
                                </div>
                                <div class="flex-shrink-0 ranking-content-in-right">
                                    ...
                                    <p class="font-15 color-heading text-center">
                                        @if(get_currency_placement() == 'after')
                                            {{ $level->to }} {{ get_currency_symbol() }}
                                        @else
                                            {{ get_currency_symbol() }} {{ $level->to }}
                                        @endif
                                    </p>
                                </div>
                            </div>
